...
/// Maps a human-facing tc-chat **room id** to the opaque **channel id** that is
/// actually used as the mistlib swarm topic. tc-chat keeps the room id as the
/// display/shareable name but joins the swarm under this derived id, so the raw
/// room name is never the on-wire topic. Anything that wants to meet tc-chat in
/// a room (e.g. the `stream relay` VRChat bridge) must join the *same* derived
/// channel — so this MUST stay byte-for-byte identical to tc-chat's
/// `channelIdFor` (tc-chat `src/lib/util.ts`): a cyrb128 digest of the room id,
/// rendered as `tcch-<32 hex>`. Not cryptographic (the algorithm is public, so
/// a known room name is still derivable) — it normalizes and obscures the
/// topic, nothing more. Only apply it to tc-chat rooms; mistl's own internal
/// rooms (mailbox/ai, [`DEFAULT_ROOM`]) are joined by their raw names.
pub fn channel_id_for(room_id: &str) -> String {
    let mut h1: u32 = 1779033703;
    let mut h2: u32 = 3144134277;
    let mut h3: u32 = 1013904242;
    let mut h4: u32 = 2773480762;
    // Iterate UTF-16 code units to match JS `String.charCodeAt`.
    for k in room_id.encode_utf16() {
        let k = k as u32;
    ...
    h3 ^= h1;
    h4 ^= h1;
    format!("tcch-{h1:08x}{h2:08x}{h3:08x}{h4:08x}")
}
...
mod tests {
    ...
    /// Golden vectors generated by tc-chat's `channelIdFor` (src/lib/util.ts).
    /// If these ever drift, the relay silently stops meeting tc-chat, so they
    /// are pinned here as the cross-language contract.
    #[test]
    fn channel_id_matches_tc_chat() {
        assert_eq!(channel_id_for("global"), "tcch-004c862ab204e9a7d2f2fb96466a20da");
        assert_eq!(channel_id_for("my-team"), "tcch-0d935599879a25e6e3855682ec262ff0");
        assert_eq!(
            channel_id_for("mistl-mailbox-v1"),
            "tcch-86f1e57a47287dbdef4ae7db00f983de"
        );
        assert_eq!(channel_id_for("a"), "tcch-5eb8e4e408dee1678f5f05b958089485");
    }

    #[test]
    fn channel_id_is_deterministic_and_shaped() {
        assert_eq!(channel_id_for("room-1"), channel_id_for("room-1"));
        assert_ne!(channel_id_for("room-1"), channel_id_for("room-2"));
        let ch = channel_id_for("room-1");
        assert!(ch.starts_with("tcch-"));
        assert_eq!(ch.len(), "tcch-".len() + 32);
    }
}
